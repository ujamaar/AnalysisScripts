...
def main():
                ...
                if(replace_previous_versions_of_plots == True):
                    ...
                    for i in xrange(len(behavior_file)):
                        if(behavior_file[i] == '_'):
                            ...
                            break
                    for plot_dirpath, plot_dirnames, plot_files in os.walk(output_directory_path + '/' + mouse_ID + '/distance_trace_plots'):                    
                        for plot_file in plot_files:
                            if plot_file.endswith(mouse_ID_and_date + '_distance_trace_plots.pdf'):  
    ...
    for behavior_and_traces_file_path in file_names:
        ...
        read_data_and_generate_plots(behavior_and_traces_file_path,output_directory_path)
...
def read_data_and_generate_plots(behavior_and_traces_file_path,output_directory_path):
...
#    trace_matrices = [list(numpy.zeros((total_number_of_cells,laps_in_environment[],dtype='float')) for ref_var in xrange(number_of_reference_variables)]
    ...
    for env in xrange(number_of_environments):
    ...
    pdf_output_directory_path = output_directory_path + '/' + mouse_ID  + '/distance_trace_plots'
    if pdf_output_directory_path:
        if not os.path.isdir(pdf_output_directory_path):
            os.makedirs(pdf_output_directory_path)

    pdf_filename = pdf_output_directory_path + '/' + mouse_ID_and_date + '_distance_trace_plots.pdf'
    ...
    for cell in xrange(5):
        ...
        trace_matrices = [list(numpy.zeros((laps_in_environment[env]*2,max_distance_frames),dtype='float')) for env in xrange(number_of_environments)]

        #trace_matrices = [list(list([] laps_with_this_adjusted_odor[odor])) for odor in xrange(len(adjusted_odors))]

        trace_data_for_this_cell = trace_data[cell,:]
        for env in xrange(number_of_environments):
            env_lap_count = 0
            for unknown_env in xrange(total_number_of_laps):
                if(environment_sequence[unknown_env] == env+1):
                    current_lap = unknown_env
                    ...
                    if(all_laps_start_frames[current_lap+1]-all_laps_start_frames[current_lap] < max_distance_frames):
                        ...
                        trace_matrices[env][env_lap_count*2  ][:] = numpy.append(distance[all_laps_start_frames[current_lap]:all_laps_start_frames[current_lap+1]],empty_part_of_distance_at_the_end)                            
                        ...
                        trace_matrices[env][env_lap_count*2+1][:] = numpy.append(trace_data_for_this_cell[all_laps_start_frames[current_lap]:all_laps_start_frames[current_lap+1]],empty_part_of_traces_at_the_end)
                        env_lap_count += 1                    
        graph_this_cell(pp,mouse_ID_and_date,cell,trace_matrices,number_of_environments,adjusted_odors,odor_sequence_in_letters,sequence_of_environments,sequence_of_lap_counts,sequence_of_track_lengths,total_number_of_cells,odor_start_and_end_points)          
...
def graph_this_cell(pp,mouse_ID_and_date,cell_index,trace_matrices,number_of_environments,adjusted_odors,odor_sequence_in_letters,sequence_of_environments,sequence_of_lap_counts,sequence_of_track_lengths,total_number_of_cells,odor_start_and_end_points):


    number_of_subplots = number_of_environments       
    ...
    if(number_of_subplots > 1):
        ...
        for ax in axs:
            current_subplot += 1
            ax = generate_subplot(ax,current_subplot,trace_matrices,adjusted_odors,odor_sequence_in_letters,odor_start_and_end_points)
    elif(number_of_subplots == 1):
        ax = generate_subplot(axs,0,trace_matrices,adjusted_odors,odor_sequence_in_letters,odor_start_and_end_points)

#    plt.show()
    pp.savefig()
    plt.close(fig)

def generate_subplot(ax,current_subplot,trace_matrices,adjusted_odors,odor_sequence_in_letters,odor_start_and_end_points):
    ...
    if(current_subplot < len(adjusted_odors)):
        ...
        ax.set_ylabel('no odor env',rotation='horizontal',horizontalalignment='right',color='red',fontsize='xx-small')        
