...
def main():
    ...
    for behavior_and_events in file_names:
        ...
        for name_letter in range (1,file_length):
            if(behavior_and_events[file_length - name_letter] == '/'):
                directory_path_pvalue_files_for_reference_variables = behavior_and_events[0:(file_length - name_letter)]
                break
        ...
        read_data_and_generate_plots(behavior_and_events, distance_bin_size,speed_threshold,gaussian_filter_sigma,lower_threshold_for_activity,split_laps_in_environment,pvalue_filenames,pvalue_threshold,mouse_plot_output_directory_path)
...
def read_data_and_generate_plots(file_path, distance_bin_size,speed_threshold,gaussian_filter_sigma,lower_threshold_for_activity,split_laps_in_environment,pvalue_filenames,pvalue_threshold,mouse_plot_output_directory_path):
...
#    initial_drops = behavior_and_event_data[5,:]
    reward_window = behavior_and_event_data[6,:]
...
#                    current_bin = env_starts_at_this_bin[ lap_count[frame] / 50] + distance[frame] / distance_bin_size
    ...
    ### for each pvalue file, generate a matrix new named place_data_each_cell
    ### and plot graphs for this matrix
    pvalue_files = [list(numpy.loadtxt(pvalue_filenames[ref_var], dtype='float', comments='#', delimiter=',',skiprows=0)) for ref_var in xrange(len(pvalue_filenames))] 


    for pvalue_file in xrange(len(pvalue_files)): 
        cell_pvalues = pvalue_files[pvalue_file]    
...
def generate_plots(file_path, place_data_each_cell,total_time_per_bin,number_of_environments, laps_in_environment, total_number_of_cells, bins_in_environment, env_starts_at_this_bin,env_track_lengths,odor_sequence_in_letters,distance_bin_size,odor_start_and_end_points,speed_threshold,total_number_distance_bins_in_all_env,gaussian_filter_sigma,lower_threshold_for_activity,average_speed_per_bin,split_laps_in_environment,mouse_plot_output_directory_path,pvalue_filename):
