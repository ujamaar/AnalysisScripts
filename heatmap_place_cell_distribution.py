...
def main():
    ...
    place_cell_distribution_bin_size = 500 #distance bin in mm
...
#        else:
        read_data_and_generate_plots(mouse_data,odor_response_time_window, distance_bin_size,speed_threshold,gaussian_filter_sigma,lower_threshold_for_activity,split_laps_in_environment,place_cell_distribution_bin_size)
...
def read_data_and_generate_plots(file_path,odor_response_time_window, distance_bin_size,speed_threshold,gaussian_filter_sigma,lower_threshold_for_activity,split_laps_in_environment,place_cell_distribution_bin_size):
    ...
    place_cell_distribution_bins_in_environment = np.zeros(number_of_environments,dtype='int')
    env_starts_at_this_bin = np.zeros(number_of_environments+1,dtype='int')
    place_cell_distribution_env_starts_at_this_bin = np.zeros(number_of_environments+1,dtype='int')
    ...
    for env in range(0,number_of_environments):
        ...
        place_cell_distribution_bins_in_environment[env] = env_track_lengths[env] / place_cell_distribution_bin_size
        place_cell_distribution_env_starts_at_this_bin[env+1] = place_cell_distribution_env_starts_at_this_bin[env] + place_cell_distribution_bins_in_environment[env]
    ...
    print 'Number of place cell distribution bins in each enviornment:'    
    print place_cell_distribution_bins_in_environment
    print 'New environment starts at place cell distribution bin:'
    print place_cell_distribution_env_starts_at_this_bin
...
#    np.savetxt(file_path.replace('.csv','_gaussian_filtered_events_per_second_ranked.csv'), place_data_each_cell, fmt='%f', delimiter=',', newline='\n')            
    ...
    for plot_env in range (0,number_of_environments):
        ...
        place_cell_distribution_data = data_array[:,0:2*number_of_environments]       
#        np.savetxt(file_path.replace('.csv','_plotted_data_sorted_for_env_%d.csv'%plot_env), place_cell_distribution_data, fmt='%f', delimiter=',', newline='\n')   

        place_cells_in_each_bin = np.zeros(np.sum(place_cell_distribution_bins_in_environment),dtype='float')

        for env in range(0,number_of_environments):
            for cell in range(0,place_cell_distribution_data.shape[0]):
                bin_for_this_place_cell = (place_cell_distribution_data[cell][2*env] * distance_bin_size ) / place_cell_distribution_bin_size
                if(place_cell_distribution_data[cell][2*env+1] >= lower_threshold_for_activity):
                    place_cells_in_each_bin[bin_for_this_place_cell] = place_cells_in_each_bin[bin_for_this_place_cell] + 1.00
        

        for place_bin in range(0,len(place_cells_in_each_bin)):
            place_cells_in_each_bin[place_bin] = place_cells_in_each_bin[place_bin]*100 / total_number_of_cells
            
#        np.savetxt(file_path.replace('.csv','_place_cell_distribution_%d.csv'%plot_env), place_cells_in_each_bin, fmt='%f', delimiter=',', newline='\n') 
        ...
        for env in range (0,number_of_environments):
            ...
            plt.figtext(0.30,0.91, "%s"%od[0], fontsize='large', color=plot_color[plot_env], ha ='left')
            plt.figtext(0.46,0.91, "%s"%od[1], fontsize='large', color=plot_color[plot_env], ha ='left')
            plt.figtext(0.62,0.91, "%s"%od[2], fontsize='large', color=plot_color[plot_env], ha ='left')            
...
#                env_aligned_to_itself_figs.append(fig)                
        ...
        ###############################################################################
        ############## generate plots for place cell distribution #####################
        ###############################################################################
        ...
        for env in range (0,number_of_environments):
            ...
            if(split_laps_in_environment == 5050):            
                fig2.suptitle('%s    env %d %s plotted against env %d %s' %(name_of_this_file, env+1,earlierORlater[env],plot_env+1,earlierORlater[plot_env]))
            elif(split_laps_in_environment == 1212):            
                fig2.suptitle('%s    env %d %s plotted against env %d %s' %(name_of_this_file, env+1,oddOReven[env],plot_env+1,oddOReven[plot_env]))
            else:
                fig2.suptitle('%s    env %d place cell distribution vs. env %d' %(name_of_this_file, env+1,plot_env+1))  

            ax2  = plt.subplot2grid((5,5), (0,0), rowspan=5,colspan=4)
...
###############################################################################
            ...
            proportion_of_place_cells_per_region = place_cells_in_each_bin[place_cell_distribution_env_starts_at_this_bin[env]:place_cell_distribution_env_starts_at_this_bin[env+1]]
            #np.savetxt(file_path.replace('.csv','_histogram_heights.csv'), proportion_of_place_cells_per_region, fmt='%f', delimiter=',', newline='\n')  
            
            max_histogram_height = max(proportion_of_place_cells_per_region)
            plt.bar(range(place_cell_distribution_bins_in_environment[env]),proportion_of_place_cells_per_region, width=1, color='g',linewidth=0)
...
###############################################################################
            ...
            if(env_track_lengths[env] == 4000):
                ...
                plt.setp(ax2,xticklabels=['0','0.5','1','1.5','2','2.5','3','3.5','4'],visible=True)
            elif(env_track_lengths[env] == 8000):
                ax2.set_xlabel('Distance(m)')
                plt.setp(ax2,xticklabels=['0','1','2','3','4','5','6','7','8'],visible=True)
            else:
                ax2.set_xlabel('Distance bin (%d mm each)'%distance_bin_size)
                plt.setp(ax2.get_xticklabels(),visible=True)

            ax2.yaxis.set_major_locator(MaxNLocator(integer=True)) 
            ax2.set_ylabel('Percentage of active cells')       
            plt.xlim (0,place_cell_distribution_bins_in_environment[env])
            plt.ylim (0,data_array.shape[0]) 
            plt.ylim (0,max(10,max_histogram_height))
#            plt.gca().invert_yaxis()

                        
            #draw white lines to mark odor regions
            plt.axvline(x=odor_start_and_end_points[env*6+0]*place_cell_distribution_bins_in_environment[env]/env_track_lengths[env], linewidth=0.2, color='r')
            plt.axvline(x=odor_start_and_end_points[env*6+1]*place_cell_distribution_bins_in_environment[env]/env_track_lengths[env], linewidth=0.2, color='r')
            plt.axvline(x=odor_start_and_end_points[env*6+2]*place_cell_distribution_bins_in_environment[env]/env_track_lengths[env], linewidth=0.2, color='r')
            plt.axvline(x=odor_start_and_end_points[env*6+3]*place_cell_distribution_bins_in_environment[env]/env_track_lengths[env], linewidth=0.2, color='r')
            plt.axvline(x=odor_start_and_end_points[env*6+4]*place_cell_distribution_bins_in_environment[env]/env_track_lengths[env], linewidth=0.2, color='r')
            plt.axvline(x=odor_start_and_end_points[env*6+5]*place_cell_distribution_bins_in_environment[env]/env_track_lengths[env], linewidth=0.2, color='r')

            fig2.add_subplot(ax2)       
            plt.show()  #can be commented out to stop showing all plots in the console
            histogram_figs.append(fig2)              
            plt.close()
...
###############################################################################
    ...
    if len(figs) > 0:
        pdf_name = file_path.replace("behavior_and_events.csv","place_cells_%1.2f.pdf"%lower_threshold_for_activity)        
        if(split_laps_in_environment == 5050):            
            pdf_name = file_path.replace("behavior_and_events.csv","place_cells_earlier_vs_later_%1.2f.pdf"%lower_threshold_for_activity)  
        elif(split_laps_in_environment == 1212):            
            pdf_name = file_path.replace("behavior_and_events.csv","place_cells_odd_vs_even_%1.2f.pdf"%lower_threshold_for_activity)
