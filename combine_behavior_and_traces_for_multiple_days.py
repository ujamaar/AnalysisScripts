...
def main():
    ...
    for dirpath, dirnames, files in os.walk(data_files_directory_path):
        for behavior_file in files:
            if behavior_file.endswith('behavior_1_day.csv'):
    ...
    for behavior_file in file_names:  
        ...
        #detect all the behavior.csv files in the folder
        behavior_files_by_day = []
        for day_dirpath, day_dirnames, day_files in os.walk(imaging_files_directory_path):
            for day_behavior_file in day_files:
                if day_behavior_file.endswith('_day.csv'):
                    behavior_files_by_day.append(os.path.join(day_dirpath, day_behavior_file))
